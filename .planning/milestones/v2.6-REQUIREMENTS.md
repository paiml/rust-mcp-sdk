# Requirements Archive: v2.6 AI-Package Portability
...
| PKGR-01 | Phase 124 | Complete | **Shipped 2026-08-27 (tag `v2.19.1`).** The coverage gate now discovers workspace-EXCLUDED publishable crates by filesystem scan (member count 24 -> 25, so `crates/pmcp-package` is covered) and machine-checks that `pmcp-package`'s publish step precedes all four of its consumers (D-10); `pmcp-openapi-server` is present in both CLAUDE.md's publish order and `release.yml`. All 14 `expected_new` crate versions confirmed live on crates.io per `124-expected-release.json`. Verified 4/4 in `124-VERIFICATION.md`. Note the release JOB failed mid-order on a crates.io ownership 403 (`pmcp-team-servers`); 11 of 14 published and 3 were recovered by hand — ownership is a publish precondition no in-repo check covers, recorded in `124-07-SUMMARY.md`. |
