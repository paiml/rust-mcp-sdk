...
must_haves:
  truths:
    ...
    - "`cargo run --example s44_server_skills` and `cargo run --example c10_client_skills` both exit 0, with c10 asserting on the `Skills::entries()` PROJECTION — the same value a `skills/list` response carries — in place of the retired index read, and stating in its output and its module doc that it demonstrates the projection rather than an RPC round trip, since the example holds no client transport (D-03, D-08, ROADMAP SC#4)."
    - "Neither example, nor any documentation surface this plan touches, claims to have exercised `skills/list` or `skills/get` over MCP; each names `tests/skills_routing.rs` as the authoritative end-to-end proof."
...
**What the blast-radius check does and does not establish (R-30).** It enumerates
**in-repo** consumers by grepping tracked source. It cannot establish that no
downstream consumer exists — a repository grep has no visibility outside the
repository. Removal is authorized by D-08 and ROADMAP success criterion 2 as an
intentional, published behavior break, and the SUMMARY must describe it that way.
The earlier phrasing — "no external consumer known", offered as a reason removal was
safe — inverted the evidence: an absence of observation is not an observation of
absence, and the authorization was never contingent on it.

## Cross-AI review disposition (125-REVIEWS.md) — findings owned by 125-04

| ID | Source | Severity | Finding | Disposition |
|---|---|---|---|---|
| R-27 | Codex (orchestrator-corroborated) | HIGH | `examples/c10_client_skills.rs` cannot call `skills/get` as written — its flow accepts only `&dyn ResourceHandler`, `skills/get` is not a `ResourceHandler` method, and the projection helpers are crate-private | **INCORPORATED — Task 2 item 2 rewritten.** Re-verified during the replan: `sep_2640_flow(handler: &dyn ResourceHandler, skill: &Skill)` at `examples/c10_client_skills.rs:93`, and `main` at `:194-220` builds `Skills::new().add(skill).into_handler()` and drives the handler in-process — the example has no client, no server and no transport. Resolution = the reviewer's honest option: demonstrate `Skills::entries()`, which IS the projection `skills/list` carries, and SAY SO rather than implying an RPC. See R-29 for the documentation half. |
| R-28 | Codex | MEDIUM | `s44` cannot inspect entries after moving registrations into the builder unless the registry is retained or cloned first — the example chains individual `.skill(...)` calls directly into `Server::builder()` | **INCORPORATED** — Task 2 item 1 now specifies the exact restructure, including the constraint that forces its shape: `bootstrap_skill_and_prompt` is the only route from an example to the dual-surface prompt, because `SkillPromptHandler` is `pub(crate)` (`src/server/skills.rs:584`) and `examples/` is a separate crate. |
| R-29 | Codex | MEDIUM | A direct `Skills::entries()` demonstration is not an RPC demonstration; documentation must not claim the examples exercised the methods over MCP | **INCORPORATED** — both examples and the two chapters state plainly that they show the projection, and name `tests/skills_routing.rs` as the authoritative end-to-end proof. This is the difference between an example that teaches and one that misleads: a reader who copies c10 and expects a working `skills/get` client has been told something false. |
| R-30 | Codex | LOW | The blast-radius statement cannot establish the absence of external consumers | **INCORPORATED** — see the paragraph directly above; the SUMMARY records an accepted, authorized break rather than a safety proof. |
| R-31 | Codex | LOW | Grep acceptance criteria intentionally retain the retired URI in negative tests, so raw occurrence counts cannot distinguish an ordinary mention from live behavior | **INCORPORATED** — the implementation-region check is re-anchored structurally (on the `#[cfg(test)]` boundary) instead of on a hardcoded line number, and the exact-count criteria become floors plus a stated allowance for explanatory prose. |
...
5. Do NOT introduce a legacy feature gate. The blast-radius check in this plan's
  ...
  <acceptance_criteria>
    - <!-- planner-discipline-allow: index.json --> The IMPLEMENTATION region of `src/server/skills.rs` names the retired URI zero times in CODE, checked with a STRUCTURAL anchor rather than a line number and with comment lines stripped (R-31): `sed -n '1,/^#\[cfg(test)\]/p' src/server/skills.rs | grep -v '^\s*//' | grep -c 'index.json'` returns 0. Two rejected alternatives, both of which this criterion previously contained in some form: a hardcoded `1,NNN` window is rejected because this task deletes roughly twenty lines from the file, so a window written before the edit measures a different region after it; and an unfiltered count is rejected because a retirement note in a rustdoc comment is a legitimate, even desirable, thing for the executor to leave behind, and a gate that forbids explaining a removal is a gate that pushes toward silent removals.
    - <!-- planner-discipline-allow: build_discovery_index_json --> `grep -v '^\s*//' src/server/skills.rs | grep -c 'build_discovery_index_json'` returns 0. The literal is unavoidable in this task's `<action>` — an instruction to delete a function must name the function — so the comment filter is what keeps the criterion from being invalidated by a comment recording the deletion.
    - <!-- planner-discipline-allow: index_json --> `grep -v '^\s*//' src/server/skills.rs | grep -c 'index_json'` returns 0, same reasoning.
    ...
    - The `src/server/builder.rs` assertion asserts absence, not deletion: `grep -c 'index.json' src/server/builder.rs` returns at least 1, and every occurrence is inside an absence assertion or an explanatory comment. A floor rather than an exact count is deliberate — an exact `== 1` breaks the moment someone adds a one-line comment explaining why the assertion is inverted, which is a change that improves the file (R-31).
  ...
  <reversibility rating="costly">
    ...
    host that reads it — an intentional, published break, authorized by D-08 and by
    ROADMAP success criterion 2. The blast-radius check enumerated the twelve in-repo
    consumers so none is left broken; it establishes nothing about consumers outside
    this repository, and the authorization never depended on it (R-30). Restoring the
    resource is a revert of one commit; flagged, not gated.
  </reversibility>
...
<task type="auto">
  ...
  <read_first>
    ...
    - examples/c10_client_skills.rs — the whole file, in particular the index read and its two `assert_eq!` at :107-114. This example ASSERTS, so it panics rather than printing wrong output. Note especially the signature `sep_2640_flow(handler: &dyn ResourceHandler, skill: &Skill)` at :93 and `main` at :194-220: the example builds a registry, calls `into_handler()`, and drives the handler IN PROCESS. There is no client, no server and no transport anywhere in this file.
    - src/server/skills.rs:388-389 — `#[derive(Default, Clone, Debug)] pub struct Skills`, and the `entries(&self)` / `into_handler(self)` pair. `entries` borrows and `into_handler` consumes, so `entries()` must be called FIRST; no clone is needed for that ordering.
    - src/server/skills.rs:584 — `pub(crate) struct SkillPromptHandler`. It is crate-private, so an example CANNOT register the dual-surface prompt directly; `bootstrap_skill_and_prompt` is the only route. This constrains the s44 restructure below — read it before choosing a shape.
    - src/server/mod.rs:4503-4530 — `ServerBuilder::skill` and `ServerBuilder::skills`, which merge into one pending registry. Registering the same skill through BOTH `.skills(registry)` and `.bootstrap_skill_and_prompt(same_skill, ...)` would present a duplicate URI at build time.
    - .planning/phases/125-sep-2640-conformance-skills-list-skills-get/125-01-SUMMARY.md and 125-02-SUMMARY.md — the exact `SkillEntry` accessor names and the fact, recorded in 125-02, that the methods reach only the streamable-HTTP `Server` path.
    - tests/skills_routing.rs — as 125-01 and 125-02 left it. This is the authoritative end-to-end `skills/list` / `skills/get` proof, and both examples must point readers at it rather than implying they are it.
...
1. `examples/s44_server_skills.rs`: remove the discovery-index line from the doc
   ...
   story: state that a server built this way answers `skills/list` and `skills/get`
   over streamable HTTP, and print the count of conforming entries the registry
   produced along with the first entry's URI and digest.

   **The restructure that makes the entry printout possible (R-28).** The example
   currently chains `.skill(...).skill(...).bootstrap_skill_and_prompt(...)` straight
   into `Server::builder()`, so no `Skills` value survives for `entries()` to be
   called on. Build a separate `Skills` registry value holding all three of the same
   `Skill` values (`Skill` derives `Clone`), call `entries()` on it, and print from
   the result — leaving the existing registration chain intact. Do NOT try to pass
   that registry to `.skills(...)` and keep `bootstrap_skill_and_prompt` as well:
   both register the code-mode skill, and the merged registry would present a
   duplicate URI at build time. Do NOT try to replace `bootstrap_skill_and_prompt`
   with a direct `.prompt(...)` call either — `SkillPromptHandler` is `pub(crate)`
   (`src/server/skills.rs:584`) and unreachable from `examples/`. Add a short comment
   at the projection registry naming both constraints, so the apparent duplication
   reads as forced rather than careless.

   Confirm the three embedded SKILL.md files already carry frontmatter with names
   matching their resolved URIs — verified at replan time, they do (`hello-world`,
   `refunds` under `.with_path("acme/billing/refunds")` resolving to final segment
   `refunds`, and `code-mode`), so no fixture edit should be needed. If a mismatch is
   found, correct the frontmatter, never the registration.

2. `examples/c10_client_skills.rs`: replace the index read and its two `assert_eq!`
   with an **honest projection demonstration**, not a simulated RPC (R-27, BLOCKING).

   **Why the obvious edit is impossible.** The pre-review instruction said to replace
   the index read with "the `skills/get` equivalent". There is none available here.
   `sep_2640_flow` takes `&dyn ResourceHandler` (`:93`), `skills/get` is not a
   `ResourceHandler` method, and this example holds no client, no server and no
   transport — `main` builds a registry, calls `into_handler()`, and drives the
   handler in process. Reaching a real `skills/get` would mean converting c10 into an
   HTTP client/server harness, which is a different example, not an edit to this one.

   Do this instead. In `main`, build the registry as `let registry = Skills::new().add(skill.clone());`
   and call `registry.entries()` BEFORE `registry.into_handler()` — `entries` borrows
   and `into_handler` consumes, so that ordering needs no clone. Assert on the single
   returned entry: its `uri` equals the registered SKILL.md URI, its
   `frontmatter` carries `name == "code-mode"`, its `resources` has four elements
   (SKILL.md plus the three references), and `resources[0].digest` matches the
   `sha256:` + 64-lowercase-hex shape. Keep the example ASSERTING rather than only
   printing: an example that panics on regression is the strongest of the three doc
   surfaces, and that property is why this file is the load-bearing edit.

   **State the boundary in the example itself (R-29).** Print, and say in the module
   doc, that this is the projection a `skills/list` response carries — the same
   `SkillEntry` values, obtained directly from the registry — and that the example
   does NOT exercise `skills/list` or `skills/get` over MCP, because it holds no
   transport. Name `tests/skills_routing.rs` as where the end-to-end wire proof
   lives. A reader who copies this file expecting a working `skills/get` client has
   been misled by an example that was trying to look more complete than it is.

   Update the surrounding comment that describes `resources/list` as returning
   "SKILL.md + index ONLY", and the Flow A paragraph in the module doc that says
   capable hosts "see SKILL.md + index entries".
...
3. Run both examples end to end and confirm the printed output describes the current
  ...
  <verify>
    <automated>bash -o pipefail -c "cargo build -p pmcp --all-features --examples 2>&amp;1 | tail -5"</automated>
  ...
  <acceptance_criteria>
    ...
    - `examples/c10_client_skills.rs` calls `entries()` on the registry BEFORE `into_handler()` consumes it, and asserts on the returned entry's `uri`, its `frontmatter` name, its four-element `resources` manifest, and its first digest's shape.
    - `examples/c10_client_skills.rs` still contains at least three `assert_eq!` or `assert!` calls — the example remains self-checking.
    - Both examples state, in their module doc AND in their printed output, that they demonstrate the entry projection rather than an RPC round trip, and both name `tests/skills_routing.rs` as the end-to-end proof (R-29). Check by asserting each file's text names that test file: `grep -c 'skills_routing' examples/c10_client_skills.rs examples/s44_server_skills.rs` returns a nonzero count for each.
    - `examples/s44_server_skills.rs` builds a `Skills` value it can call `entries()` on, and carries a comment naming the two constraints that force the shape (the duplicate-URI merge and `SkillPromptHandler` being crate-private).
  ...
  <done>
Both examples run clean, print and assert on the real entry projection, name no
retired URI, honestly disclaim demonstrating the RPC they cannot reach, and point
readers at the test that does. Committed.
  </done>
</task>
...
1. `pmcp-book/src/ch12-8-skills.md`: rewrite the four index-naming sites to describe
   ...
   not listable. Replace the index-read snippet at ~:266 with a snippet whose shape
   matches what `examples/c10_client_skills.rs` now does, so the chapter and the
   example teach the same call.

   **Keep the chapter's claim exactly as strong as the code it shows (R-29).** The
   snippet demonstrates the `Skills::entries()` projection, not an RPC round trip.
   Show the wire shape of a `skills/list` response as JSON prose so a reader learns
   what a host receives, and state plainly that the runnable Rust beside it produces
   that same projection locally rather than issuing the call. A chapter that presents
   an in-process registry read as though it were an MCP request teaches the reader a
   client API that does not exist.
