...
The following changes are observable by a downstream consumer, but **every symbol
involved is private**, so `cargo semver-checks` reports no semver update
required. This section is the only place a consumer learns of them. Each is
recorded in the project's broken-windows ledger (`.planning/WINDOWS.md`) under
the entry id given, and each is marked there with the `[CONSUMER-OBSERVABLE]`
sentinel.
...
- **WINDOWS.md entry 13 — `Client::dispatch_request` discards mis-addressed
  frames.** It now compares `response.id` against the request id it is awaiting
  and keeps looping on a mismatch, discarding the mismatched frame. This is
  strictly better than one caller silently receiving another caller's tool result
  (a cross-request data leak inside one process), but it is a real cost. Note
  that a **lenient server that re-types the id** — a JSON string where the client
  sent a number — triggers it, because `RequestId` equality is typed and
  structural and JSON-RPC 2.0 requires the response id to be the same *value* as
  the request's.
...
- **WINDOWS.md entry 20 — a bounded discard introduces a new failure mode.**
  ...
  your peer is slow and slightly lenient, you will observe it. Separately, the
  transport write lock is now released and re-taken periodically during a
  discard, so another operation on the same `Client` waits at most one slice
